...
namespace TimeOfDay
{
    [Serializable] public class TOD_AtmosphericScattering
    {
    ...
    #region [Fields]
        ...
        // General Settings.
        [SerializeField] 
        TOD_ShaderQuality m_ShaderQuality = TOD_ShaderQuality.PerVertex;
        
        [SerializeField] 
        TOD_MoonRayleighMode m_MoonRayleighMode = TOD_MoonRayleighMode.OpossiteSun;
        
        [SerializeField] 
        TOD_UpdateType m_BetaRayUpdate = TOD_UpdateType.OnInit;
        
        [SerializeField] 
        TOD_UpdateType m_BetaMieUpdate = TOD_UpdateType.OnInit;

        // Params.
        [SerializeField] 
        TOD_AtmosphericScatteringParams m_Params = new TOD_AtmosphericScatteringParams
        {
            applyFastTonemap       = true,
            contrast               = 0.3f,
            groundColor            = Color.gray,
            wavelength             = TOD_Wavelength.EarthValues2,
            tickness               = 1.0f,
            ...
            sunMie                 = TOD_MiePhase.Default,
            moonMie                = TOD_MiePhase.Default
        };
        ...
        /// <summary></summary>
        public TOD_MoonRayleighMode MoonRayleighMode => m_MoonRayleighMode;
    ...
    #region [Properties]
        ...
        public float DayIntensity => (TOD_Mathf.Saturate(SunDir.y + 0.40f) * m_Params.sunBrightness); 

        /// <summary> Sunset/Dawn atmosphere horizon. </summary>
        public float SunsetDawnHorizon => TOD_Mathf.Saturate(Mathf.Clamp(1.0f - (SunDir.y), 0.0f, 1f)); 
        ...
        public float NightIntensity
        {
            get
            {
                ...
                {
                    case TOD_MoonRayleighMode.OpossiteSun:
                        return TOD_Mathf.Saturate(-SunDir.y + 0.25f);

                    case TOD_MoonRayleighMode.CelestialContribution:
                        return TOD_Mathf.Saturate(MoonDir.y + 0.25f);
                }
                return 0.0f;
            }
        }
    ...
    #region [Initialize]
        ...
        {
            if(m_BetaRayUpdate == TOD_UpdateType.OnInit)
                SetBetaRay();
            
            if(m_BetaMieUpdate == TOD_UpdateType.OnInit)
                SetBetaMie();
        }
    ...
    #region [SetParams]
        ...
        public void SetGlobalParams()
        {
            ...
            {
                case TOD_ShaderQuality.PerVertex: 
                
                Shader.DisableKeyword(TOD_Keywords.atms_PerPixelAtmosphere); 
                
                break;
                
                case TOD_ShaderQuality.PerPixel:  
                
                Shader.EnableKeyword(TOD_Keywords.atms_PerPixelAtmosphere);  
                
                break;
            }

            if(m_Params.applyFastTonemap)
                Shader.EnableKeyword(TOD_Keywords.atms_ApplyFastTonemaping);
            else
                Shader.DisableKeyword(TOD_Keywords.atms_ApplyFastTonemaping);

            Shader.SetGlobalFloat(TOD_PropertyIDs.atms_ContrastID, m_Params.contrast);
            Shader.SetGlobalColor(TOD_PropertyIDs.atms_GroundColorID, m_Params.groundColor);

            /// Rayleigh.
            ////////////////////
            Shader.SetGlobalColor(TOD_PropertyIDs.atms_SunAtmosphereTintID, SunAtmosphereTint); 

            switch(m_MoonRayleighMode)
            {
                case TOD_MoonRayleighMode.OpossiteSun:
                    Shader.EnableKeyword(TOD_Keywords.atms_EnableMoonRayleigh);
                    Shader.SetGlobalInt(TOD_PropertyIDs.atms_MoonAtmosphereTintID, 0);
                break;

                case TOD_MoonRayleighMode.CelestialContribution:
                    Shader.EnableKeyword(TOD_Keywords.atms_EnableMoonRayleigh);
                    Shader.SetGlobalInt(TOD_PropertyIDs.atms_MoonAtmosphereTintID, 1);
                break;

                case TOD_MoonRayleighMode.Off: 
                
                    Shader.DisableKeyword(TOD_Keywords.atms_EnableMoonRayleigh); 
            ...
            (
                TOD_PropertyIDs.atms_MoonAtmosphereTintID, 
                m_Params.moonAtmosphereTint * 
                NightIntensityMultiplier
            );

            Shader.SetGlobalFloat(TOD_PropertyIDs.atms_AtmosphereHazinessID, m_Params.atmosphereHaziness);
            Shader.SetGlobalFloat(TOD_PropertyIDs.atms_AtmosphereZenithOffsetID, m_Params.atmosphereZenithOffset);
            Shader.SetGlobalFloat(TOD_PropertyIDs.atms_SunsetDawnHorizonID, SunsetDawnHorizon);
            Shader.SetGlobalFloat(TOD_PropertyIDs.atms_RayleighZenithLengthID, m_Params.rayleighZenithLength);
           
            if(m_BetaRayUpdate == TOD_UpdateType.Realtime)
                SetBetaRay();

            if(m_BetaMieUpdate == TOD_UpdateType.Realtime)
                SetBetaMie();

            Shader.SetGlobalVector(TOD_PropertyIDs.atms_BetaRayID, BetaRay * m_Params.tickness);
            Shader.SetGlobalVector(TOD_PropertyIDs.atms_BetaMieID, BetaMie);
            Shader.SetGlobalFloat(TOD_PropertyIDs.atms_DayIntensityID, DayIntensity);
            Shader.SetGlobalFloat(TOD_PropertyIDs.atms_NightIntensityID, NightIntensity);

            /// Mie.
            //////////////////
            Shader.SetGlobalFloat(TOD_PropertyIDs.atms_MieZenithLengthID, m_Params.mieZenithLength);

            // Sun
            Shader.SetGlobalColor(TOD_PropertyIDs.atms_SunMieTintID, m_Params.sunMie.tint);
            Shader.SetGlobalFloat(TOD_PropertyIDs.atms_SunMieAnisotropyID, m_Params.sunMie.anisotropy);
            Shader.SetGlobalFloat(TOD_PropertyIDs.atms_SunMieScatteringID, m_Params.sunMie.scattering);
            Shader.SetGlobalVector(TOD_PropertyIDs.atms_PartialSunMiePhaseID, PartialSunMiePhase);
            
            // Moon
            Shader.SetGlobalColor(TOD_PropertyIDs.atms_MoonMieTintID, m_Params.moonMie.tint);
            Shader.SetGlobalFloat(TOD_PropertyIDs.atms_MoonMieAnisotropyID, m_Params.moonMie.anisotropy);
            Shader.SetGlobalFloat(TOD_PropertyIDs.atms_MoonMieScatteringID, m_Params.moonMie.scattering * MoonPhasesIntensityMultiplier);
            Shader.SetGlobalVector(TOD_PropertyIDs.atms_PartialMoonMiePhaseID, PartialMoonMiePhase);
    ...
    #region [Compute]
        ...
        {
            return TOD_Mathf.k_3PI16 * (1.0f + cosTheta * cosTheta);
        } 
        ...
        public float MiePhase(float g, float cosTheta)
        {
            ...
            return  TOD_Mathf.k_InvPI4 * (PHG.x *  ((1.0f + cosTheta * cosTheta) * Mathf.Pow(PHG.y-(PHG.z*cosTheta),-1.5f)));
        }
        ...
        public void CustomOpticalDepth(float pos, out Vector2 srm)
        {
            ...
            pos = TOD_Mathf.Saturate(pos * m_Params.atmosphereHaziness);
        ...
        public Vector3 ComputeAtmosphericScattering(Vector3 inCEF, float sunCosTheta, Vector3 sunMiPhase, Vector3 moonMiePhase)
        {
            ...
            if(m_MoonRayleighMode != TOD_MoonRayleighMode.Off)
            {
                Vector3 moonScatter;
